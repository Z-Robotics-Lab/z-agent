...
These tests verify that:
    1. `import zeno.ros2` never crashes regardless of rclpy availability.
...
def _reload_ros2_module() -> types.ModuleType:
    """Force a fresh import of zeno.ros2 by removing cached modules."""
    # Remove cached submodules to allow re-evaluation of the guard
    to_remove = [k for k in sys.modules if k.startswith("zeno.ros2")]
    for key in to_remove:
        del sys.modules[key]
    return importlib.import_module("zeno.ros2")
...
class TestROS2ImportNoCrash:
    """Importing zeno.ros2 must never raise an exception."""
    ...
    def test_import_vector_os_top_level_no_crash(self) -> None:
        ...
        import zeno  # noqa: F401

    def test_reimport_is_safe(self) -> None:
        """Re-importing zeno.ros2 multiple times must not crash."""
        for _ in range(3):
            mod = _reload_ros2_module()
            assert mod is not None
...
class TestROS2NodesGuardedWithoutROS2:
    """When ROS2 is not installed, node classes must not be importable from zeno.ros2."""
...
class TestROS2NodeFiles:
    ...
    def _node_path(self, filename: str) -> str:
        ...
        return os.path.join(base, "zeno", "ros2", "nodes", filename)
...
class TestROS2LaunchFile:
    ...
    def _launch_path(self) -> str:
        import os
        base = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
        return os.path.join(base, "zeno", "ros2", "launch", "nano.launch.py")
