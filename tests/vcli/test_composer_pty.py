...
"""Bare-``zeno`` PTY acceptance for the framed input composer.
...
def test_bare_repl_frame_multiline_help_and_lf_submit() -> None:
    ...
    assert "zeno>" in text  # stable acceptance marker inside the frame
    assert "Enter" in text and "Alt+Enter" in text and "Tab" in text
    assert "› /help" in text  # submitted frame collapsed to compact transcript
