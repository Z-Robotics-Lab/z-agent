...
# ROUND_DEADLINE_EPOCH · pipe GOAL.md+ROUND.md (every 10th: REVIEW.md) to the adapter ·
# then sim-teardown, check.sh --post (fail => quarantine), heartbeat, sleep INTERVAL.
#
...
while :; do
  ...
  PROMPT=loop/ROUND.md
  [ $(( N % 10 )) -eq 0 ] && PROMPT=loop/REVIEW.md   # reviews are FED, not remembered
  export ROUND_N="$N" ROUND_DEADLINE_EPOCH=$(( $(date +%s) + ${ROUND_TIMEOUT:-3600} ))
  LOG="loop/.state/round-$N.log"
  echo "=== R$N ($PROMPT) — deadline $(date -d "@$ROUND_DEADLINE_EPOCH" '+%H:%M:%S') — log $LOG ==="

  if command -v systemd-run >/dev/null 2>&1 && systemd-run --user --scope --collect true >/dev/null 2>&1; then
    cat loop/GOAL.md "$PROMPT" | systemd-run --user --scope --collect --quiet \
    ...
    cat loop/GOAL.md "$PROMPT" | setsid timeout --foreground "${ROUND_TIMEOUT:-3600}" "$ADAPTER" >"$LOG" 2>&1 &
    P=$!; wait "$P" || true
