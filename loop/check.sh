...
declare -A CAP=( [AGENTS.md]=130 [STATUS.md]=40 [docs/LESSONS.md]=150 [loop/ROUND.md]=120
                 [loop/GOAL.md]=15 [loop/REVIEW.md]=60 [docs/verify.md]=80
                 [loop/README.md]=100 )
RULES_CAP=50                 # docs/rules/*.md each
WIRING_CAP=60                # docs/wiring/*.md each
WIRING_MAX_FILES=8
...
for p in docs/rules/*.md; do
  [ -f "$p" ] && { [ "$(wc -l <"$p")" -le "$RULES_CAP" ] || f "$p over ${RULES_CAP} lines"; }
done
if compgen -G 'docs/wiring/*.md' >/dev/null; then
  for p in docs/wiring/*.md; do
    [ "$(wc -l <"$p")" -le "$WIRING_CAP" ] || f "$p over ${WIRING_CAP} lines"
  done
  n=$(ls docs/wiring/*.md | wc -l)
  [ "$n" -le "$WIRING_MAX_FILES" ] || f "docs/wiring/ has $n cards (max $WIRING_MAX_FILES)"
fi
...
if [ -f loop/checks_allowlist.txt ]; then
  ...
  done < <(git ls-files '*.md' | grep -E '^([^/]+\.md|docs/[^/]+\.md|docs/(rules|wiring)/[^/]+\.md|loop/[^/]+\.md|loop/ledger/[^/]+\.md)$')
...
if git rev-parse -q --verify HEAD~1 >/dev/null 2>&1; then
  ...
  # ---------- 4. Spine gate (tamper-EVIDENT, not tamper-proof — docs/rules/gates.md) ---
...
ROT_MD=$(git ls-files 'docs/*.md' 'docs/rules/*.md' 'docs/wiring/*.md' 'loop/*.md' AGENTS.md README.md 2>/dev/null | grep -v '^docs/DECISIONS.md$')
...
python3 loop/checks_anchors.py || f "wiring-card anchors stale (ANCHOR-FAIL lines above)"
