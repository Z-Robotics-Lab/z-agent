...
5. Sim safety: ONE sim at a time; tear down after each via `scripts/sim-teardown`; NEVER
   `pkill mujoco`; tests only via `scripts/run-tests` (memory-capped — 50 test files spawn
   sims; unbounded pytest OOMs the host). READ docs/RULES.md sim-safety BEFORE any sim.
...
- docs/DECISIONS.md: RULINGS only ([RULING] tag; admission test in docs/RULES.md).
  Append-only, never rewritten; corrections = new entry with a forward link.
- docs/LESSONS.md: refuted paths / hazards / recipes / frontier / hidden-bug casebook —
  terse, pointer-terminated (D#/E#/hash).
...
Do NOT read docs/DECISIONS.md, docs/ARCHITECTURE.md, or docs/WIRING.md at cold start —
...
Details + exec-summary template: docs/RULES.md CEO-gates. A solo cloner is their own CEO —
the gate still means: stop, decide deliberately out-of-loop, record a [RULING]. Gate/approval
...
- Every headline claim passes docs/RULES.md red-team BEFORE it is recorded anywhere.
...
- Not a loop round (e.g. a PR)? Changing behaviour still means: update the WIRING section
  you touched, add a ledger row if you verified, follow loop/ROUND.md §5 for RECORD.

## Pull on demand (read the moment the task touches it; paths are the API)
any sim run / headline claim / non-obvious bug / gate / touching docs / editing code →
docs/RULES.md (sections: sim-safety · red-team · debugging · CEO-gates · doc-governance ·
standards) · verify/accept → docs/VERIFY.md · design/ladder → docs/ARCHITECTURE.md ·
subsystem wiring → docs/WIRING.md · a cited D# → docs/decisions-index.md then `git show` ·
hidden-bug casebook → docs/LESSONS.md · bring a skill / add a tool / producer & permissions
→ docs/reference.md · run the loop → loop/README.md · install/product → README.md.
