...
11. **Embodiments are CONFIG, not code.** A robot morphology is added by a per-robot **config file** — a manifest of every morphology-specific parameter (spawn pose, DoF/joint layout, sensor mounts, nominal stance, gait/policy ref), the way a ROS2 nav-stack ships per-robot param YAMLs — plus its assets (URDF/mesh/policy). NEVER a bespoke per-robot driver class, NEVER morphology constants hardcoded in Python. There is ONE generic embodiment driver; every body conforms **uniformly** to `BaseProtocol` (no per-robot method drift — `disconnect`/`name`/`get_velocity`/`get_odometry`/`supports_*` are the same for all). Each logical capability (locomotion, navigation, lidar, scene-build, perception) has exactly ONE shared, config-parameterized implementation — never a per-embodiment fork (the go2-vs-g1 split this rule exists to kill: two navigate impls, three lidar copies, two scene builders). **If adding or changing a robot needs an edit to kernel or driver code, the design is wrong — move the difference into config.**
12. **Testing is layered; end-to-end is MANDATORY and never skipped.** Every capability carries unit + integration + **end-to-end** tests; the e2e test exercises the whole pipeline in the real simulator through bare `vector-cli` + NL (the honest verdict surfaces in that conversation). A green unit suite is necessary but is NEVER acceptance. Every embodiment / skill / capability added or refactored is e2e-proven through the bare REPL before it is "done" — for a refactor, that means each affected body still passes its e2e flow.
