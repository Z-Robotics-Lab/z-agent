...
### SIM MEMORY — HARD RULE (OOM killed python 3× at ~51 GB, 2026-06-20)

A go2 sim = MuJoCo + the FULL nav stack (`launch_explore.sh`: bridge + TARE + FAR) ≈ several GB; the host is **62 GB**. Sims do NOT auto-tear-down — they STACK and OOM-kill python. **NEVER OOM again:**
- **`rosm nuke --yes` + `pkill -9 -f mujoco` after EVERY sim run/test, immediately, before the next.** Non-negotiable.
- **SERIALIZE sims GLOBALLY** — one sim at a time across ALL sessions/loops. Before launching, check `~/.claude/loops/*.lock` + `pgrep -f "mujoco|vcli.cli|launch_explore"`; if a sim or the EvolvingLoop is live, WAIT — do not add a second.
- **NEVER background a sim** (`run_in_background` + `--sim-go2`/sim PTY). Run ONE in the foreground, nuke, then `free -g` to confirm recovery before the next.
- **Pre-flight memory guard:** `free -g` before launch — need ≥ ~20 GB available; if not, nuke + recheck. Don't run the whole sim pytest suite in one long process — small batches, nuke between (MuJoCo instance leak).
