...
"""Deterministic sim-oracle verify predicates over the connected arm.

These are the callables a playground sub-goal's ``verify`` expression evaluates
against. They read the sim's DETERMINISTIC ground truth — ``get_object_positions``
/ ``get_joint_positions`` / ``fk`` on the connected arm — never the VLM. The VLM
detect/describe pipeline stays the agent's perception *skill*; the verifier
checks the oracle (ADR-008: generator and verifier independent).

Grounding contract:
- The arm is reached from the agent via ``getattr(agent, "_arm", None)`` (the
  same accessor the CLI / sim tool use). When the arm is absent or not connected,
  every predicate FAILS SAFE (returns ``False`` / ``0``) — it must NEVER raise
  into the GoalVerifier sandbox.
...
def _gripper_is_holding(agent: Any) -> bool:
    ...
    try:
        ...
        logger.debug("playground gripper.is_holding failed: %s", exc)
        return False
...
def _ee_position(arm: Any) -> list[float] | None:
    ...
    try:
        ...
        logger.debug("playground fk/get_joint_positions failed: %s", exc)
        return None
...
def make_arm_at_home(agent: Any) -> Callable[[], bool]:
    ...
    def arm_at_home() -> bool:
        ...
        try:
            ...
            logger.debug("playground get_joint_positions failed: %s", exc)
            return False
