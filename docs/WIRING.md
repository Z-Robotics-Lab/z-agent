...
- The machine verdict: `zeno/vcli/verdict.py::VerdictReport.from_trace(trace, oracle_names)` delegates `verified` VERBATIM to `evidence_passed` (contract-test-pinned). `VERDICT_SENTINEL` = "ZENO_VERDICT" (primary) + `LEGACY_VERDICT_SENTINEL` = "VECTOR_VERDICT" (dual-emitted legacy alias — D184 identity transition).
- The verdict escapes cli.main via `zeno/vcli/cli.py::run_one_turn` (the `-p/--json` non-interactive entry): `_emit` prints `report.to_sentinel_lines()` (ZENO_VERDICT + VECTOR_VERDICT, one payload) on stdout and returns `report.exit_code()` (0 verified / 2 ran-not-verified / 1 error-or-no-trace). Chat-only / error turns emit `VerdictReport.no_trace` (fail-closed).
