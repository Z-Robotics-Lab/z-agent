...
verified-against: 38f772f

**frozen — GATE-APPROVED required** (Invariant 1): every file below is honest-verify spine; do NOT edit without the CEO gate.

- A turn becomes an `ExecutionTrace` (frozen dataclasses in `zeno/vcli/cognitive/types.py`): the native producer `native_loop.py::run_turn_native` (or the legacy VGG executor) records one `StepRecord` per (action-chain → verify) pair and assembles the trace via `NativeStepRunner.build_trace`.
- The trace is graded — never re-derived — by `zeno/vcli/cognitive/trace_store.py::evidence_passed`: True iff ≥1 checked step AND every step classifies GROUNDED, plus the coordinate-goal (STEP-15) and object-goal (D17) turn gates, which can only REJECT.
...
- A verify string is structurally graded by `zeno/vcli/cognitive/evidence_classifier.py::classify_verify_expr`: GROUNDED only when a world oracle's result GATES the verdict (`evidence_classifier.py::_is_grounded_node`, recursive AST walk; rejects `... or True`, tautologies, bare state oracles, literal-container burials).
...
- World oracles are factories in `zeno/vcli/worlds/go2_sim_oracle.py::make_at_position` / `make_facing` / `make_visited` (plus arm_sim_oracle.py, g1_perception_oracle.py::make_detection_matches_gt) — bound over live sim GT the actor cannot author.
...
- The machine verdict: `zeno/vcli/verdict.py::VerdictReport.from_trace(trace, oracle_names)` delegates `verified` VERBATIM to `evidence_passed` (contract-test-pinned). `VERDICT_SENTINEL` = "VECTOR_VERDICT".
- VECTOR_VERDICT escapes cli.main via `zeno/vcli/cli.py::run_one_turn` (the `-p/--json` non-interactive entry, ~line 1857): `_emit` prints `report.to_sentinel_line()` on stdout and returns `report.exit_code()` (0 verified / 2 ran-not-verified / 1 error-or-no-trace). Chat-only / error turns emit `VerdictReport.no_trace` (fail-closed).
...
```
anchors:
zeno/vcli/cognitive/trace_store.py::evidence_passed
zeno/vcli/cognitive/trace_store.py::classify_step_evidence
zeno/vcli/cognitive/trace_store.py::verify_oracle_names
zeno/vcli/cognitive/trace_store.py::step_evidence_ok
zeno/vcli/cognitive/evidence_classifier.py::classify_verify_expr
zeno/vcli/cognitive/evidence_classifier.py::_PREDICATE_ORACLES
zeno/vcli/cognitive/evidence_classifier.py::_is_grounded_node
zeno/vcli/cognitive/actor_causation.py::grade
zeno/vcli/cognitive/actor_causation.py::capture
zeno/vcli/cognitive/actor_causation.py::is_robot_predicate
zeno/vcli/verdict.py::VerdictReport
zeno/vcli/verdict.py::VERDICT_SENTINEL
zeno/vcli/cli.py::run_one_turn
zeno/vcli/worlds/go2_sim_oracle.py::make_at_position
zeno/vcli/worlds/g1_perception_oracle.py::make_detection_matches_gt
```
...
- Invariant 3: embodiments are CONFIG, not code — a robot enters via `zeno/embodiments/<id>/robot.yaml` (go2/, g1/ today), never a kernel or driver edit.
- A robot.yaml is parsed fail-loud into frozen dataclasses by `zeno/embodiments/config.py::load_embodiment_config` → `config.py::parse_embodiment_config` → `EmbodimentConfig` (model / spawn / stance / sensors / policy / capabilities / grasp). Frozen dataclasses change ADDITIVELY only (new field last + default, Invariant 7).
- The sim drivers stand the body up and stash the manifest: `zeno/hardware/sim/mujoco_go2.py::MuJoCoGo2` and `mujoco_g1.py::MuJoCoG1` each call `load_embodiment_config("<id>")` at connect and store it as `self._config` — the handle `capability_profile._declared_profile` reads back at runtime.
- Drivers implement the duck-typed contract `zeno/hardware/base.py::BaseProtocol` (connect/walk/set_velocity/get_position/get_heading/get_lidar_scan/…) — the agent binds one as `agent._base` (plus `_arm`/`_gripper`/`_perception`).
- The DofLayout seam: `zeno/embodiments/dof_layout.py::DofLayout(model, root_body, num_actuated)` introspects the root freejoint's qpos/qvel slice addresses from the COMPILED model (replaces per-driver literals; g1's `mujoco_g1.py::_G1Offsets` is now a thin adapter subclass). `dof_layout.py::build_stance_vector` builds the nominal stance FROM the manifest's `stance:` dict in model joint order; `dof_layout.py::build_robot_geom_set` gives the lidar self-filter geom set.
- Capabilities: `config.py::CapabilityProfile` (has_base/has_arm/has_gripper/camera/lidar) is declared in robot.yaml, but the ONE runtime resolver is `zeno/embodiments/capability_profile.py::resolve_capability_profile(agent)`.
...
- Its three consumers (all gates single-sourced, can't drift): `zeno/vcli/native_loop.py::_build_motor_tools` (navigate gate on has_base; manipulation-skill gate on has_arm), `zeno/vcli/worlds/robot.py::_agent_has_camera` (detector registration), `zeno/vcli/engine.py::init_vgg` (`_has_base` → nav decompose vocab, ~line 428).
...
```
anchors:
zeno/embodiments/config.py::load_embodiment_config
zeno/embodiments/config.py::parse_embodiment_config
zeno/embodiments/config.py::EmbodimentConfig
zeno/embodiments/config.py::CapabilityProfile
zeno/embodiments/capability_profile.py::resolve_capability_profile
zeno/embodiments/capability_profile.py::_declared_profile
zeno/embodiments/dof_layout.py::DofLayout
zeno/embodiments/dof_layout.py::build_stance_vector
zeno/embodiments/dof_layout.py::build_robot_geom_set
zeno/hardware/base.py::BaseProtocol
zeno/hardware/sim/mujoco_go2.py::MuJoCoGo2
zeno/hardware/sim/mujoco_g1.py::MuJoCoG1
zeno/hardware/sim/mujoco_g1.py::_G1Offsets
zeno/vcli/engine.py::init_vgg
```
...
- The model-driven ReAct producer: `zeno/vcli/native_loop.py::run_turn_native(engine, user_message, …)` drives `engine._backend.call(messages, tools, system)` in a loop (cap `_MAX_NATIVE_TURNS=24`) and returns an `ExecutionTrace`. It NEVER computes `verified` — the caller `zeno/vcli/cli.py::run_one_turn` feeds the trace to `VerdictReport.from_trace`.
...
```
anchors:
zeno/vcli/native_loop.py::run_turn_native
zeno/vcli/native_loop.py::NativeStepRunner
zeno/vcli/native_loop.py::handle_verify
zeno/vcli/native_loop.py::_build_motor_tools
zeno/vcli/native_loop.py::_native_system_prompt
zeno/vcli/native_loop.py::should_attempt_native
zeno/vcli/native_loop.py::_NativeBaseNavigateTool
zeno/vcli/native_loop.py::_NativeDetectTool
zeno/vcli/native_loop.py::_ARM_REQUIRING_SKILLS
zeno/vcli/native_loop.py::_MUTATING_CODE_TOOLS
zeno/vcli/native_loop.py::_GRASP_SKILLS
zeno/vcli/native_loop.py::_PLACE_SKILLS
zeno/vcli/native_loop.py::_POST_PLACE_REGRASP_NUDGE
zeno/core/agent.py::execute_skill
zeno/core/agent.py::_skill_ends_holding
zeno/vcli/native_loop.py::_UNPRODUCTIVE_NUDGE_AT
zeno/vcli/native_loop.py::_MAX_TURNS_WITHOUT_VERIFY
zeno/vcli/native_loop.py::seen_verify_outcomes
zeno/vcli/native_loop.py::last_verify_result
zeno/vcli/tools/skill_wrapper.py::wrap_skills
zeno/vcli/tools/skill_wrapper.py::SkillWrapperTool
zeno/vcli/worlds/robot.py::persona_blocks
zeno/vcli/worlds/robot.py::register_capabilities
zeno/vcli/cli.py::run_one_turn
```
