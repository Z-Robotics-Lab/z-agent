...
updated: 2026-06-19
phase:   **STEP 6 COMMITTED + PUSHED (ae90386 on origin/feat/orchestrator-redesign) — DEV-WORLD ACTION LEAK CLOSED, ORCHESTRATOR-RE-VERIFIED. native_loop._build_motor_tools(agent, engine) now offers the engine registry's "code"-category tools (file_read/write/edit/bash/glob/grep, via _code_tools_from_registry — single-sourced from engine._registry.list_categories, rule 3) ALONGSIDE wrap_skills(agent) robot skills (navigate still excluded); world-agnostic BY CONSTRUCTION (native asks the engine for its registered tools, NO 'if dev' branch, rule 7); skills layered after code tools so a world skill wins any clash. I re-verified: dev C.1 through the REAL cli.main --native-loop PTY → strategy='file_write', verify=path_contains, GROUNDED, verified True, exit 0, file written (the leak is CLOSED — test renamed ..._covers_dev_action_via_native + assertions flipped leak→coverage; C.2 world-agnostic proof kept); arm scripted robot test still PASS (3.7s — code tools don't break robot routing); chunked suite 1209 + 3 deepseek reds; spine + engine.py BYTE-UNCHANGED; no permission bypass (execution still flows dispatch_skill→tool.execute under --no-permission); no new dep; no AI attribution. ===> NATIVE NOW COVERS ALL THREE WORLDS: go2 walk/turn/multi-step + arm pick + dev file_write — all real-verified honest, graded by the UNCHANGED spine. The CUTOVER is FULLY PREREQUISITED (native covers everything the world-agnostic fast-path does). blocked: CUTOVER (make native the DEFAULT turn path + staged fast-path deletion) = CEO gate, EXECUTIVE SUMMARY PRESENTED to Yusen 2026-06-19, AWAITING APPROVAL — do NOT flip the default or delete anything until approved (the summary's 'dev stays legacy' caveat is now resolved — dev is covered). NEXT (non-gated pivot while cutover awaits) = an M2 capability with a clean predicate: ORIENT into the go2 ExploreSkill → if its motion is cleanly actor-CAUSED (visited is a _BASE_PREDICATE) prove explore→visited/room_coverage routing through native; if explore drives via cmd_vel (gated out of actor-causation, like navigate) pivot to oracle i18n (close the step-4 cross-language grasp finding — make holding_object/at_position accept localized names). DECISION QUEUE: CUTOVER (CEO gate, pending Yusen); nav→FAR needs actor-causation extended to cmd_vel + FAR colcon DQ-15; VLN→SysNav DQ-16; NL embodiment SWITCH needs a 'which embodiment' verify predicate; oracle i18n.**
