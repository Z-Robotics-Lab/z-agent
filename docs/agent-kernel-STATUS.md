...
updated: 2026-06-19
phase:   **M1 STEP 1 COMMITTED + PUSHED (d10cfce on origin/feat/orchestrator-redesign) — the strangler-fig native tool-use producer, ORCHESTRATOR-RE-VERIFIED (not a self-report). `vcli/native_loop.py` (run_turn_native + NativeStepRunner) imports ONLY the spine (actor_causation/trace_store/GoalVerifier-namespace/SkillWrapperTool; import-firewall test + my AST grep enforce NO goal_decomposer/goal_executor/strategy_selector/vgg_harness). engine.run_turn_native = 32-line thin delegate. Flag `--native-loop` + `VECTOR_NATIVE_LOOP=1` (default OFF) branches cli.run_one_turn; verdict block UNCHANGED. The R2b trichotomy REPRODUCES through the NEW producer on the real go2 sim, re-run BY ME (8m06s): HONEST walk->verify(at_position) -> True/GROUNDED/exit0/strategy=walk; NO-OP verify-only -> UNCAUSED/RAN/False/exit2; TELEPORT (test-only qpos-poke skill, cmd_motion≈0) -> UNCAUSED/RAN/exit2. ONE StepRecord per (action->verify) pair; capture-before-first-skill / grade-fresh-after-verify. Spine BYTE-UNCHANGED. Suite 1195 passed flag ON and OFF, re-run by me (3 deepseek .env reds tolerated). DELETED NOTHING; no AI attribution. NEXT = M1 STEP 2 = the LIVE-LLM real-sim acceptance: does the REAL frontier model (repo .env), given NL "走到 (x,y)" + the go2 tool-set, AUTONOMOUSLY issue walk->verify(at_position) and earn GROUNDED? Step 1 proved the PLUMBING (scripted backend); step 2 proves the THESIS (model owns decompose/route/replan). That earns the cutover (native default for go2-walk) -> THEN the Phase-2 strangle order per next-prompt.md fix 4 -> M2 (NL embodiment switch + routing breadth TARE/FAR/SysNav/VLA).**
