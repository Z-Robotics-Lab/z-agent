...
updated: 2026-06-19
phase:   **M1 FIRST STEP BUILT (uncommitted — orchestrator commits) — the strangler-fig native tool-use producer. `vcli/native_loop.py` (run_turn_native + NativeStepRunner) imports ONLY the spine (actor_causation/trace_store/GoalVerifier-namespace/SkillWrapperTool; import-firewall test enforces NO goal_decomposer/goal_executor/strategy_selector/vgg_harness). engine.run_turn_native = 32-line thin delegate. Flag `--native-loop` + `VECTOR_NATIVE_LOOP=1` (default OFF) branches cli.run_one_turn; the verdict block is UNCHANGED. The R2b trichotomy REPRODUCES through the NEW producer on the real go2 sim: HONEST walk->verify(at_position) -> verified True/GROUNDED/exit0; NO-OP verify-only -> UNCAUSED/RAN/verified False/exit2; TELEPORT (test-only qpos-poke skill) -> UNCAUSED/RAN/exit2. ONE StepRecord per (action-chain->verify) pair; capture-before-first-skill / grade-fresh-after-verify timing mirrors GoalExecutor. Spine BYTE-UNCHANGED (git diff empty). Suite 1195 passed flag ON and OFF (3 deepseek .env reds tolerated). DELETED NOTHING. NEXT = M2 (NL embodiment switch + routing breadth TARE/FAR/SysNav/VLA); Phase-2 strangle order per next-prompt.md fix 4.**
