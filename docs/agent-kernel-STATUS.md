...
updated: 2026-06-19
phase:   **STEP 5 COMMITTED + PUSHED (ae60b0b on origin/feat/orchestrator-redesign) — native-attempt-then-fallback ROUTING SPIKE, flag default OFF, ORCHESTRATOR-RE-VERIFIED. cli.run_one_turn: `_native_first_enabled` (--native-first / VECTOR_NATIVE_FIRST=1, independent of --native-loop) → try run_turn_native; `_native_trace_acted(trace)` (≥1 step non-empty strategy) → emit native verdict; ZERO action (couldn't route, e.g. dev world no agent) → FALL THROUGH to the legacy decompose+execute block (no half-action side-effects — fallback fires only on zero dispatch). I re-verified: fallback-wiring 12 passed (native-acts→native, no-action→legacy, both via call-spy), s53/s54 19 passed (REPL untouched), chunked suite 1209 passed (+12 new) + 3 deepseek reds flag ON and OFF, covered-go2 real-sim under --native-first routes to native → GROUNDED (240s, by me). Spine + native_loop.py + engine.py BYTE-UNCHANGED; no keyword router (rule 1); no AI attribution. FINDINGS (for the cutover): (a) DOUBLE-LLM cost on fallback (full native ReAct + full legacy decompose for an unroutable goal — ~2x spend/latency in the transition); (b) session pollution benign in -p (one-shot) but a fallback on a persisted session would leave the abandoned native attempt's turns in history → snapshot/throwaway-session on fallback; (c) the empty-.strategy seam can't distinguish 'native acted but failed' from 'didn't act' (currently impossible — pin as invariant). ===> CUTOVER IS A CEO GATE, NOW PENDING YUSEN. blocked: CUTOVER (make native the DEFAULT turn path for covered goal shapes + staged fast-path deletion) = cross-package data-flow change → EXECUTIVE SUMMARY PRESENTED to Yusen 2026-06-19, awaiting approval; do NOT flip the default or delete anything until approved. PIVOT (non-gated, while cutover awaits) = close the dev-world wiring leak (surface the dev code toolset as native action tools so cli.main --native-loop acts in the dev world; flips test C.1 leak→covered; real-verify via the dev PTY) — also a cutover prerequisite for FULL fast-path deletion. DECISION QUEUE: CUTOVER (CEO gate, pending); M2 breadth (NL embodiment SWITCH — needs a 'which embodiment' verify predicate; explore→TARE via room_coverage/visited; nav→FAR needs actor-causation extended to cmd_vel + FAR colcon DQ-15; VLN→SysNav DQ-16); oracle i18n (step-4 cross-language grasp finding).**
