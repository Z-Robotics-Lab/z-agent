...
updated: 2026-06-19
phase:   **M1 STEP 2 COMMITTED + PUSHED (a5391eb on origin/feat/orchestrator-redesign) — the LIVE-LLM real-sim acceptance of run_turn_native, ORCHESTRATOR-RE-VERIFIED on the real go2 sim (not a self-report). The REAL product model (DeepSeek deepseek-v4-flash via repo .env; a forced OpenRouter claude-sonnet-4-6 also routed → routing is a DESIGN property) given NL "走到 (x,y)" + the go2 tool-set AUTONOMOUSLY issues walk->verify(at_position) — proven across MY runs #2+#3 (2 consecutive 2/2-green) on tests/vcli/test_native_loop_live_llm_pty.py (NEW; double-gated: real key + VECTOR_LIVE_LLM=1; @sim so excluded from the routine suite). The test hard-asserts routing=walk+at_position on EVERY step + ≥1 genuinely-GROUNDED caused step (un-fakeable by no-op/teleport) + full verified=True only for a clean one-walk trace. FINDING (honest): the default model ROUTES reliably but initially did NOT RECOVER (my run #1: a single walk under-shot → RAN, model didn't replan → 0 grounded → flaky). FIX = strengthened `native_loop.py` _native_system_prompt only (force RECOVER: repeat walk->verify until at_position PASS, never finish on FAIL, command ~2x/≥1.5m) — runner UNCHANGED, no replan bookkeeping; the model now autonomously replans (case2 trace shows 2-3 walks RAN…RAN…GROUNDED). The moat stayed HONEST throughout: multi-walk turns honestly report whole-turn RAN; only a clean one-walk grounds verified=True (no false green). pty_cli.py gained a minimal `live=` param (scripted callers byte-identical). Spine BYTE-UNCHANGED; suite 1195 passed + 3 deepseek reds (live test SKIPPED w/o opt-in); DELETED NOTHING; no AI attribution. NEXT = M1 STEP 3 (subsume-before-delete): prove native routes a MULTI-skill / multi-step go2 task (model picks the right skill + the right verify predicate per sub-goal + recovers) on the real sim, AND covers a DEV-world single-skill goal (the world-agnostic fast-path's responsibility) — both honest prerequisites to STEP 4 = make native default + delete the _try_skill_goal_tree/_verify_for_skill fast-path (tripwire go2 PTY + dev PTY + s53/s54 green). THEN Phase-2 strangle order (next-prompt.md fix 4) → M2 (NL embodiment switch + routing breadth; navigate cmd_vel is gated out of actor-causation → honest FAR needs actor-causation extended to the cmd_vel route first).**
