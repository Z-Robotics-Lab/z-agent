...
updated: 2026-06-30 · D143 — [LOOP running, OpenRouter-independent] Qwen pipeline CONFIRMED e2e (R1 D141: qwen-plus routing + Qwen3-VL-plus judge, both DashScope; Clash global->rule). R2 D142: near-red GT 4/5 + eyes 0/5. R3 D143: near eyes gap CLOSED — capture.py near verdict snapshot now az270 side view (Go2 clearly upright; az135 read the grasp's pitch as "lying" -> judge FAILed). Eyes now PASSes upright near frames; REMAINING near gap = GT routing variance (qwen-plus ~0.8). R4 IN-PROGRESS: qwen-max near-red N=5 for >=0.9 GT. Tooling: measure_qwen.py (direct-cli, NO PTY — PTY SIGKILLs the cli -9); run ALL inline cmds with `set +e` (errexit aborts on pkill no-match); DashScope reachable in DEFAULT sandbox (rule-mode). Prior: PLACE grounds e2e (D135) bottles reliable (D136/D138). EVERY ROUND: git commit.
