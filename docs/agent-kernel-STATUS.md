...
updated: 2026-06-30 · D148 — [LOOP R1-R6, OpenRouter-INDEPENDENT pipeline: Qwen routing + Qwen3-VL judge, both DashScope; Clash global->rule]. SCOREBOARD (measure_qwen.py, qwen-max routing + qwen3-vl-plus judge + az270 capture, N=5): NEAR red 5/5 green 5/5 blue 4/5 = 0.93 MET(>=0.9); FAR red 4/5 green 3/5 blue 3/5 = 0.67 BELOW(0.8). eyes_accept==grounded_rate everywhere (az270 fixed the near "pitched=lying" miscal D143; no GT-grounded+vision-FAIL disagreements; on RANs GT+eyes AGREE = moat healthy). Two D142 gaps RESOLVED: eyes(az270 D143) + near routing(qwen-max D144). REMAINING gap = FAR (0.67). R7 (D149/D150) diagnosed TWO FAR modes: (1) nav-not-at-far-table = GT-RAN+eyes-FAIL (dominant in measure runs), (2) ik_unreachable = GT-RAN+eyes-PASS (far_diag; arm short at table). NEXT R8: ONE correlated FAR run (per-step diagnosis + eyes verdict together) to size each mode, then skill-level fix (mode1 nav/recovery to far standoff; mode2 approach/nudge reach _POST_APPROACH_NUDGE_N) -> FAR>=0.8 all-3. Diag tool: scratchpad/run_far_diag.sh (full -u cli log). Tooling: measure_qwen.py (direct-cli, NO PTY — PTY SIGKILLs cli -9 w/ Qwen, backlog); ALL inline cmds need `set +e` (errexit aborts on pkill no-match); DashScope reachable in DEFAULT sandbox (rule-mode); loop env ~/.claude/loops/vector-evolve.env (QWEN_MODEL=qwen-max). PLACE floor not yet re-measured on new pipeline. EVERY ROUND: git commit.
