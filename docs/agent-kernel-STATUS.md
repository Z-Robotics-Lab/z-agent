...
updated: 2026-06-30 · D152 — [LOOP R1-R9, OpenRouter-INDEPENDENT pipeline DELIVERED: Qwen routing(qwen-max) + Qwen3-VL-plus judge, both DashScope; Clash global->rule]. SCOREBOARD (measure_qwen.py + per-trial diag + az270): NEAR red 5/5 green 5/5 blue 4/5 = 0.93 MET(>=0.9); FAR ~0.70 (green 7/10, red 4/5, blue 3/5) HIGH-VARIANCE+multi-mode BORDERLINE(<0.8); PLACE green 1/3 NOT met. eyes_accept==grounded_rate for fetch (az270 fixed near "pitched=lying" D143; on RANs eyes+GT agree=moat healthy); PLACE had the 1st GT-grounded+eyes-FAIL disagreement (D151, wide post-place stance). RESOLVED: eyes(az270 D143)+near routing(qwen-max D144). OPEN GAPS: (1) FAR ~0.70 — high-variance, 3 modes no_detections/ik_unreachable/ran_no_weld (D152); needs LARGER N + variance reduction across modes, NOT one fix. (2) PLACE 1/3 — pick+place unreliable on qwen-max route + post-place verdict frame ambiguous (D151). NEXT: FAR variance-reduction (steadier far standoff/seat/weld-radius) and PLACE reliability — each a debug round; measure_qwen.py prints per-trial diag to size modes. Tooling: direct-cli NO PTY (PTY SIGKILLs cli -9 w/Qwen, backlog); ALL inline cmds need `set +e`; DashScope reachable in DEFAULT sandbox (rule-mode); loop env ~/.claude/loops/vector-evolve.env (QWEN_MODEL=qwen-max). EVERY ROUND: git commit.
