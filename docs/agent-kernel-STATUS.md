...
updated: 2026-06-19
phase:   **M1+ STRANGLE PROVEN — native producer subsumes the planner across 3 worlds + cross-language, CUTOVER PENDING YUSEN (CEO gate). Pushed on origin/feat/orchestrator-redesign: M0 (R1 honest gate ae2d0ea + R2a PTY verdict eada1bf + R2b actor-causation bda1047) → M1 run_turn_native (d10cfce plumbing, a5391eb live go2-walk thesis, c37c64f multi-skill walk+turn + dev-leak finding) → step4 arm pick (1ec2436) → step5 native-attempt-then-fallback routing spike (ae60b0b) → step6 dev code-tool coverage (ae90386) → step7 cross-language grasp vocab (309c4a3). NATIVE PRODUCER (vcli/native_loop.py; honest spine BYTE-UNCHANGED — trace_store/actor_causation/evidence_classifier/verdict/goal_verifier/types/goal_executor) NOW COVERS, all ORCHESTRATOR-RE-VERIFIED on the real cli.main PTY / sim, graded by the unchanged moat: go2 walk (live+recover) / turn→facing (CAUSED) / multi-step journey (live) ; arm pick→holding_object (CAUSED via gripper-weld) ; dev file_write→path_contains (world-agnostic via engine registry code tools) ; cross-language (Chinese cmd → canonical holding_object('banana'), oracle stays strict). Routing = model-driven, no keyword router (rule 1). native-attempt-then-fallback (cli.run_one_turn, --native-first/VECTOR_NATIVE_FIRST default OFF): covered→native, zero-action→legacy fallback. STEP 8 REVIEW (adversarial milestone review, Workflow w187m6n0q, 6 skeptics): FOUND + I-CONFIRMED + FIXED a CRITICAL MOAT HOLE (HOLE 1, commit d0e7814) — the evidence classifier graded a truthy-constant SHORT-CIRCUIT verify (`at_position(99,99) or True`) as GROUNDED (it checked an oracle was CALLED, not that its RESULT gated the verdict); now classify_verify_expr requires every truth-bearing leaf to be oracle-gated (_is_grounded_node, STRICTER per rule 5), +19-case regression test, trichotomy re-verified 3/3 on real go2 sim post-fix, suite 1233. HOLE 2 (gripper causation lacks a commanded-motion conjunct) I REPRODUCED as NOT exploitable (bare gripper_close at home grasps nothing → holding_object False → UNCAUSED; real scene doesn't pre-position objects at the EE) → deferred defense-in-depth. Docs-hygiene (abeffc9): deleted 3 stale pre-redesign agent-kernel phase plans + ARCHITECTURE.md redesign-status note. Architecture verdict = SOLID (native_loop modular, import-firewall holds, engine thin delegate). DEFERRED MOAT GAPS = the next hardening target: GOAL-AUTHENTICITY for non-robot/state predicates — a shape-trivial `len(get_position())==3` compare + a dev `file_exists('/etc/passwd')` no-op both classify GROUNDED with NO actor-causation (actor-causation today grades only robot base/arm/gripper predicates). ===> CUTOVER (make native the DEFAULT turn path for covered shapes + staged reversible fast-path→REPL→should_use_vgg→decomposer deletion) = CROSS-PACKAGE DATA-FLOW CEO GATE. blocked: EXECUTIVE SUMMARY PRESENTED to Yusen 2026-06-19, AWAITING APPROVAL — do NOT flip the default or delete anything until approved; every round re-check his response. NEXT (non-gated, while cutover awaits) = STEP 9 GOAL-AUTHENTICITY moat hardening (close the deferred gaps above: a state-compare triviality guard + actor-causation/causation-tie for state+dev predicates so a verify against pre-existing/invariant state grades RAN) — the moat is the crown jewel, the review just proved it had a hole, so harden it next; stricter-only + regression + real-verify. DECISION QUEUE (mostly CEO-gated): CUTOVER (pending Yusen); nav→FAR + explore→TARE need actor-causation→cmd_vel (R2b deferred) + nav-stack colcon DQ-15; VLN→SysNav DQ-16; place needs gripper-release causation; NL embodiment SWITCH needs a 'which-embodiment' predicate. Real SO-101 arm gated on `ls /dev/ttyACM*` (absent — sim only).**
