...
updated: 2026-06-19
phase:   **STEP 4 COMMITTED + PUSHED (1ec2436 on origin/feat/orchestrator-redesign) — native producer SUBSUMES the 3rd capability class (ARM/GRIPPER manipulation), ORCHESTRATOR-RE-VERIFIED on the real arm sim. The arm embodiment is `cli --sim` (MuJoCoArm); arm skills = get_default_skills() (Home/Pick/Place/Gripper…). (A) tests/vcli/test_native_loop_arm_pty.py @sim — scripted on real MuJoCoArm via run_turn_native: pick→verify(holding_object('banana')), verify_result True, actor=CAUSED (gripper-weld 0→1 channel), GROUNDED, verified True; I re-ran (3.8s PASS). (B) tests/vcli/test_native_loop_arm_live_pty.py @sim @live_llm — the REAL model autonomously routes pick→holding_object('banana') on the real arm sim; I re-ran 2/2 GROUNDED/verified True. FINDINGS: (1) SO-101 MuJoCoArm has NO ctrl_motion → arm_at_home grades UNCAUSED on --sim; only the GRIPPER-WELD channel grades CAUSED → pick (not home) is the moat-honest arm capability. (2) [I caught on re-verify] the implementer's nudge had WEAKENED verify to no-arg holding_object() ('holding anything' — looser, rule-5 violation: any object satisfies a specific-object goal); I REVERTED it to a strict QUOTED holding_object('<name>') nudge + made the live instruction ENGLISH so the strict predicate matches the scene's English label. (3) holding_object(target) is English-label-exact → Chinese object-specific grasp verify fails despite a real grasp (moat correct, brittle) → cross-language object grasp verification needs oracle i18n (DEFERRED, do NOT paper over by weakening). native_loop.py change = _native_system_prompt grasp-oracle guidance only (strict, quoted); spine BYTE-UNCHANGED; chunked suite 1197+3 deepseek reds; DELETED NOTHING; no AI attribution. NATIVE NOW COVERS: go2 walk/turn/multi-step (single+multi skill, recover) + arm pick (manip), all real-verified honest; world-agnostic producer proven; dev-world ACTION wiring still leaks (step-3 finding). NEXT = drive toward the CUTOVER (the redesign payoff): a non-gated ROUTING SPIKE (native-attempt-then-fallback, flag stays default-OFF, real-verified for covered go2+arm goals + clean fallback for uncovered/dev), THEN a CUTOVER EXECUTIVE SUMMARY for Yusen — the default-turn-path flip + fast-path deletion is a CROSS-PACKAGE DATA-FLOW CEO GATE, present + PAUSE, never cross autonomously. DECISION QUEUE: CUTOVER (CEO gate); full fast-path deletion needs the dev-world wiring leak closed first (non-gated); M2 breadth (NL embodiment SWITCH — needs a verify predicate for 'which embodiment'; explore→TARE via room_coverage/visited; nav→FAR needs actor-causation extended to cmd_vel + FAR colcon DQ-15; VLN→SysNav DQ-16).**
