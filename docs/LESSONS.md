...
- New WORLD plugs in as CONFIG but a confirmed bar does NOT transfer zero-shot (R229/E52, SPLIT): go2_warehouse.xml (compact industrial box, distinct enclosure from the 20×14m house) enters via VECTOR_ROOM_TEMPLATE → build_room_scene(room_template_path=) with 0 kernel/driver edits (Inv.3), compiles with the 5 pickables + pick_table + place_bin byte-identical coords (unit 9/9; MjModel: green [10.88,3.0,0.32], dog [10.0,3.0]). BUT green fetch RAN 0/2 on the bare face (v4-flash): run1 wandered (walk fwd 2.0×N → pick 'Cannot locate target object'), run2 perception_grasp×11 never completed. Config verified LIVE + furniture byte-identical ⇒ NOT moved-geometry — perception_grasp/approach does not complete under the new visual+lidar scene. LESSON: a new-world BUILD (compiles, config-only) is a FLOOR, NOT acceptance; the deeper bar is that grasp/perception must TRANSFER, and clutter (E51, same room + decoy geoms) is far cheaper than a new enclosure. Next = Hypothesis-Loop debug (detector-sees-green? approach-stops-short on new lidar walls? camera framing?) → E52.
  R230 CAVEAT (adjudicated refuted, but confound found): the R229 warehouse runs did NOT set the local VLM route, and run2's "perception_grasp×11 never completed" matches the E53 OpenRouter-402 `look`-escalation confound EXACTLY (the R230 house control reproduced that spin without the local route). So "transfer fails on a new world" is NOT cleanly established — the warehouse 0/2 is very possibly the VLM-402 billing confound, not a world-specific perception failure. The E52-debug round MUST first re-run warehouse WITH `VECTOR_VLM_URL` local Ollama before concluding anything world-specific → E53.
...
- AMBITION CRITIC (R230 review): the loop answered the R200 local-hill critique — it pivoted to real breadth
  (embodiment E48-50, scene-clutter E51, new-world E52). But R230 exposes the deeper flaw: the ACCEPTANCE
  HARNESS is not robust — a perception-VLM 402 fails SILENTLY (spins to no-verdict, indistinguishable from a
  capability failure), so recent breadth "refutations" (E52 warehouse 0/2) are of UNKNOWN provenance. You
  cannot trust a moat whose failure mode is a silent external-billing hang. NEXT BAR (non-gated): repl_accept
  must default the local Ollama VLM route AND fail-loud on a look/describe_scene 402 — then re-open E52 → E53.
