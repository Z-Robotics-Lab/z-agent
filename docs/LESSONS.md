...
- Scene breadth via CONFIG (R226 built / R227 verified N=2, E51): VECTOR_SCENE_CLUTTER injects 5 decorative distractors (incl. a SAME-HUE off-centre green decoy) into the frozen go2 tabletop via the existing `extra_geoms` seam (same one g1's percept_target_red uses), 0 kernel/driver edits (Inv.3); a CONFIRMED green fetch (v4-flash) GROUNDED 2/2 — real bottle grasped (holding_object GT), decoy is contype/conaffinity 0 + no freejoint → weldless/unpickable so a mis-ground yields False → True=discrimination, unfakeable.
