...
## 8. P4 信息密度仪表盘 TUI（owner 选定方向 2026-07-14，进行中）

对话流 → 监控面板。full_screen prompt_toolkit App 三区：顶部常驻状态区（品牌+
身份+pose/course/odom/estop/stack）、中部可滚动回合区、底部常驻输入区。天然化解
"常驻输入框 vs 实时过程"的权衡（三区分屏共存）。显示层 only，消费同源
