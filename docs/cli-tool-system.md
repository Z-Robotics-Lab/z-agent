...
```
vector-cli  ─┐
             ├→ VectorEngine → native tool-use producer (legacy VGG = 回退) → skill.execute()
vector-os-mcp┘
```
...
(注册表驱动,D74)是 `should_use_vgg` 的已验证替代(安全超集)。本节是权威;遗留 tool_use / 回退路径(IntentRouter 关键词层、tool_use `run_turn`、VGG decompose/execute、GoalDecomposer)已折叠到下方"Legacy paths"一节。
...
> **LEGACY — 正在 S8 退役(CEO 闸门，尚未批准);详见 git history + docs/DECISIONS.md(D62 · D72–D74)。**
> 关键词路由的 `tool_use` / VGG `decompose→plan→execute→verify` producer 仍**存在**，只作回退路径
> (`VECTOR_LEGACY_TURN=1`，或 native 轮零动作时下沉)，其文件仍列在下方"文件目录"——但**默认**是
> 模型驱动的 native producer(见上方"当前 producer 架构")。原先冗长的遗留流程图(`IntentRouter` 关键词
> 分类管线、VectorEngine `run_turn()` tool_use 循环、`~52%` token 路由表、`cognitive/` VGG GoalDecomposer
> 循环)已从本文档删除以保持时效;需要时从 git 取回。
> **诚实验证脊柱**(`VerdictReport.from_trace` / `evidence_passed`，及 `cognitive/` trace/evidence/verdict)
> **不属于**此遗留层——它字节未改，为**所有** producer 共用。
...
- 工具按类别分组管理
- 运行时动态启用/禁用整个类别
- （旧路由）配合 IntentRouter 按意图只发送相关工具 — 关键词路由将在 S8 退役(pending CEO)，native producer 直接下发本世界工具集
...
| 阶段 | 策略 | 效果 |
|------|------|------|
| v1（legacy，S8 待退役 pending CEO） | 全部启用，IntentRouter 按意图路由 | 平均省 52% token |
...
验收/判定契约（`VECTOR_VERDICT` 字段表、退出码、FakeBackend 测试接缝、PTY/CI 门）已移至
**[docs/verify.md](verify.md)** —— REAL-VERIFY 的唯一 runbook。
本文档只保留开发者视角：`-p/--json` 的判定由 `vcli/verdict.py` 的 frozen `VerdictReport` 承载，只从诚实脊柱（`evidence_passed`）构建，绝不二次推导。
...
├── intent_router.py        # IntentRouter — 意图路由（关键词 → 类别，legacy，S8 退役）
...
├── cognitive/               # VGG 认知层 + 诚实验证脊柱（~24 模块）
